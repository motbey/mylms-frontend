...
import type { ImageCompareContent } from "./image-compare-types";
import { ImageCompareLearner } from "./ImageCompareLearner";
...
function summariseImageCompareForMetadata(content: ImageCompareContent): string {
  const parts: string[] = [];
  if (content.topImageAlt?.trim()) parts.push(`Before alt: ${content.topImageAlt.trim()}`);
  if (content.bottomImageAlt?.trim())
    parts.push(`After alt: ${content.bottomImageAlt.trim()}`);
  if (content.settings?.showLabels) {
    if (content.settings.topLabel?.trim())
      parts.push(`Before label: ${content.settings.topLabel.trim()}`);
    if (content.settings.bottomLabel?.trim())
      parts.push(`After label: ${content.settings.bottomLabel.trim()}`);
  }
  parts.push(`Initial percent: ${content.initialPercent ?? 50}`);
  return parts.join("\n");
}

export const ImageCompareBlock: React.FC<ImageCompareBlockProps> = (props) => {
  const {
    block,
    onChange,
    isPreviewMode = false,
  ...
  const compareContent = (block.content ?? {}) as ImageCompareContent;

  const metadataSummary = useMemo(
    () => summariseImageCompareForMetadata(compareContent),
    [compareContent]
  );

  const handleContentChange = (next: ImageCompareContent) => {
    ...
    <div className="w-full group">
      ...
      >
        ...
        <BlockWrapper layout={layout}>
          {isPreviewMode ? (
            <ImageCompareLearner blockId={block.id} content={compareContent} />
          ) : (
            <ImageCompareEditor
              blockId={block.id}
