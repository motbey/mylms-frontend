    ...
    <>
      <div className="relative min-h-screen overflow-hidden -m-8">
        ...
        <main className="relative z-10 flex items-center justify-center px-6 pb-16">
          <section className="w-full max-w-md rounded-3xl border border-white/25 bg-white/10 backdrop-blur-xl shadow-[0_25px_80px_rgba(0,0,0,0.35)]">
            ...
            <div className="relative px-8 py-10">
              ...
              {showAuthError ? (
                  <div className="text-center">
                    <h1 className="mb-4 text-center text-2xl font-bold text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.45)]">
                      Authorization Failed
                    </h1>
                    <p className="text-white text-sm text-center bg-red-500/30 p-3 rounded-lg border border-red-500/50 mb-6">
                      You are not authorized to access the admin portal.
                    </p>
                    <button 
                      onClick={handleSignOutAndHome}
                      className={auroraButtonClasses}
                    >
                      <span className="relative z-10">Sign Out & Return Home</span>
                    </button>
                  </div>
              ) : (
                <>
                  <h1 className="mb-2 text-center text-2xl md:text-3xl font-bold text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.45)]">
                    {step === 1 ? 'Admin Login' : 'Verify Admin Code'}
                  </h1>
                  ...
                  {step === 1 ? (
                    <form onSubmit={handleSendCode}>
                      <div className="mb-4">
                        <label htmlFor="email" className="mb-2 block text-sm font-medium text-white/90">Admin Email</label>
                        <input
                          id="email"
                      ...
                      <button type="submit" className={auroraButtonClasses} disabled={loading}>
                        <span className="relative z-10">{loading ? 'Sending...' : 'Send Admin Login Code'}</span>
                      </button>
                    </form>
                  ) : (
